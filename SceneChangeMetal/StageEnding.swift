...
//  SceneChangeMetal
//
//  Created by Yoshiki Izumi on 2019/07/17.
...
class StageEnding {
    ...
    init() {
        for i in 0..<STAGE_HEIGHT {
            for j in -1-STAGE_WIDTH..<STAGE_WIDTH {
                if i % 2 == 0 {
                    if j % 2 == 0 {
                        cubeData.append(Cube(r:Float.random(in:low..<1),g:Float.random(in:low..<1),b:Float.random(in:low..<1),a:1.0,sx:1.0,sy:1.0,sz:1.0))
                    } else {
                        cubeData.append(Cube(r:Float.random(in:low..<1),g:Float.random(in:low..<1),b:Float.random(in:low..<1),a:1.0,sx:1.0,sy:1.0,sz:1.0))
                    }
                } else {
    ...
    func updateLeft(characterPosition: float3) {
        var index = 0
    ...
    func updateRight(characterPosition: float3) {
        var index = 0
        for _ in 0..<STAGE_HEIGHT * 2 {
            for _ in -STAGE_WIDTH..<STAGE_WIDTH {
                if translateDataCube[index].x < characterPosition.x - Float(STAGE_WIDTH) {
    ...
    func updateGoal(characterPosition: float3) {
        var index = 0
        for i in 0..<STAGE_HEIGHT {
            for j in -1-STAGE_WIDTH..<STAGE_WIDTH {
                if translateDataCube[index].x < characterPosition.x - Float(STAGE_WIDTH) {
